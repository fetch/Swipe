...
if(!this.isScrolling){e.preventDefault();clearTimeout(this.interval);this.deltaX=this.deltaX/((!this.index&&this.deltaX>0||this.index==this.length-1&&this.deltaX<0)?(Math.abs(this.deltaX)/this.width+1):1);this.element.style.MozTransform=this.element.style.webkitTransform='translate3d('+(this.deltaX-this.index*this.width)+'px,0,0)';e.stopPropagation();}},onTouchEnd:function(e){var isValidSlide=Number(new Date())-this.start.time<250&&Math.abs(this.deltaX)>20||Math.abs(this.deltaX)>this.width/2,numSlides=Math.abs(this.deltaX)>this.width?Math.round(this.deltaX/this.width)*-1:(this.deltaX<0?1:-1),numSlides=this.index+numSlides<0?-this.index:(this.index+numSlides>this.length?this.length-this.index-1:numSlides),isPastBounds=!this.index&&this.deltaX>0||this.index+numSlides==this.length&&this.deltaX<0;if(!this.isScrolling){this.slide(this.index+(isValidSlide&&!isPastBounds?numSlides:0),this.speed);}
